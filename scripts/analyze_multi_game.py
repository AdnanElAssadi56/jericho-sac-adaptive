#!/usr/bin/env python3
"""
Analyze results across multiple games (zork1, detective, pentari)
Generate comprehensive statistics and tables for paper
"""
import json
import numpy as np
...
# Games with complete 3-seed data
...
def load_progress(game, seed, method):
    ...
    # Path relative to workspace root (script runs from workspace root)
...
def analyze_game(game):
    ...
    # Print results
    print(f"{'Rank':<6} {'Method':<30} {'Mean':<12} {'Std':<10}")
    print("-"*80)
    ...
    for i, r in enumerate(results, 1):
        ...
        print(f"{i:<6} {r['name']:<30} {r['mean']:<12.2f} {r['std']:<10.2f} ({improvement:+.1f}%)")
...
def main():
    ...
    print("Games: Zork1, Detective, Pentari (3 seeds each)")
    print("="*80)
    ...
    with open('results/data/multi_game_results.pkl', 'wb') as f:
    ...
    method_avg_score = defaultdict(list)
    ...
    for method in METHODS:
        ...
        print(f"{marker} {METHOD_NAMES[method]:<28} {wins:<10} {avg_imp:>6.1f}% ± {std_imp:.1f}%")
    
    print(f"\nResults saved to results/data/multi_game_results.pkl")
    ...
    print("\\begin{table*}[t]")
    print("\\caption{Performance across three games. Mean $\\pm$ std over 3 seeds. Best method per game in bold.}")
    ...
    print("\\toprule")
    print("Method & Zork1 & Detective & Pentari \\\\")
    print("\\midrule")
    
    # Print baselines first
    print("\\multicolumn{4}{c}{\\textit{Baselines}} \\\\")
    for method in ['baseline_no_shaping', 'baseline_static_shaping']:
        ...
        for game in GAMES:
            ...
            best_method = game_data['results'][0]['method']
            if method_result:
                if method == best_method:
                    row.append(f"\\textbf{{{method_result['mean']:.1f} $\\pm$ {method_result['std']:.1f}}}")
                else:
    ...
    # Time-based adaptive
    print("\\midrule")
    print("\\multicolumn{4}{c}{\\textit{Time-Based Adaptive}} \\\\")
    for method in ['adaptive_time_decay_exp', 'adaptive_time_decay_linear', 'adaptive_time_decay_cosine']:
        row = [METHOD_NAMES[method]]
        for game in GAMES:
            game_data = all_game_results[game]
            method_result = next((r for r in game_data['results'] if r['method'] == method), None)
    ...
    # Sparsity-based adaptive
    print("\\midrule")
    print("\\multicolumn{4}{c}{\\textit{Sparsity-Based Adaptive}} \\\\")
    for method in ['adaptive_sparsity_triggered', 'adaptive_sparsity_sensitive']:
        row = [METHOD_NAMES[method]]
        for game in GAMES:
            game_data = all_game_results[game]
            method_result = next((r for r in game_data['results'] if r['method'] == method), None)
    ...
    # Uncertainty-based adaptive
    print("\\midrule")
    print("\\multicolumn{4}{c}{\\textit{Uncertainty-Based Adaptive}} \\\\")
    for method in ['adaptive_uncertainty_informed']:
        row = [METHOD_NAMES[method]]
        for game in GAMES:
            game_data = all_game_results[game]
            method_result = next((r for r in game_data['results'] if r['method'] == method), None)
    ...
    print("Best method per game:")
    for game in GAMES:
        best = all_game_results[game]['results'][0]
        baseline = all_game_results[game]['baseline_static']
        improvement = ((best['mean'] - baseline['mean']) / baseline['mean'] * 100)
        print(f"  {game.capitalize():<15}: {best['name']:<30} ({best['mean']:.1f} ± {best['std']:.1f}, {improvement:+.1f}%)")
    
    print(f"\nAdaptive methods beating static shaping:")
    for game in GAMES:
        baseline = all_game_results[game]['baseline_static']
        adaptive_better = [r for r in all_game_results[game]['results'] 
                          if 'adaptive' in r['method'] and r['mean'] > baseline['mean']]
        print(f"  {game.capitalize():<15}: {len(adaptive_better)}/6 methods")
