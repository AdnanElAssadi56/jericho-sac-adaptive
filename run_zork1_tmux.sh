...
ROM="${SCRIPT_DIR}/dependencies/z-machine-games-master/jericho-game-suite/zork1.z5"
...
FAILED=("adaptive_time_decay_linear" "adaptive_uncertainty_informed")
for i in "${!NAMES[@]}"; do
  name="${NAMES[$i]}"
  extra="${EXTRA_ARGS[$i]}"

  # Seed 0 only for failed ones, otherwise just 1 and 3
  if [[ " ${FAILED[*]} " == *" ${name} "* ]]; then
    seeds=(0 1 3)
  else
    seeds=(1 3)
  ...
  for seed in "${seeds[@]}"; do
    outdir="${SCRIPT_DIR}/${BASE_OUT}/zork1/seed${seed}/${name}"
